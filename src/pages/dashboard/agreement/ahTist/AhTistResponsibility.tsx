...
const AhTistResponsibility = () => {
    const editor = useRef(null);
    const [content, setContent] = useState('');
    const [updateContent, { isLoading: isPending }] = useUpdateAgreementMutation();
    const { data, isLoading } = useGetAgreementQuery({ query: `?type=responsibility&for=artist` });
    ...
    const handleUpdate = async () => {
        ...
        try {
            const res = await updateContent({
                payload: { content, type: 'responsibility', for: 'artist' },
            }).unwrap();
            if (res?.success) {
    ...
    return (
        <section className="p-4 grid gap-4">
            <div className="flex justify-between items-center gap-4">
                <h1 className="text-3xl text-primary font-semibold">Ah-tist Responsibility</h1>
            </div>
...
export default AhTistResponsibility;
